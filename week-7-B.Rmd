...
```{r sc1}
fit_obj <- stan_glm(data = qscores, # Define data source
                    hours ~ 1, # Outcome on left, predictor on right.
                    family = gaussian(), # What kind of distribution
                    refresh = 0)

fit_obj

# Intercept Median is the mean of the data.
```
...
fit_obj %>%
  as_tibble() %>%
  rename(hours = "(Intercept)") %>%
  ggplot(aes(x = hours)) +
 geom_histogram(aes(y = after_stat(count / sum(count))),
                binwidth = 0.01,
                color = "white") +
  labs(x = "Hours (Median)", y = "Probability")
